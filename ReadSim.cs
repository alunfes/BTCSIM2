    ...
    public class ReadSim
    {
        ...
        public void startReadSim(int from, int to, int opt_term)
        {
            ...
            using (var sr = new StreamReader("opt nanpin.csv"))
            {
                ...
                {
                    var ele = data.Split(',');
                    //No.	num trade	win rate	realized pl	pt	lc	num_split	func	ma_term	nanpin timing	lot splits
                    ...
                    para_pt.Add(no, Convert.ToDouble(ele[4]));
                    para_lc.Add(no, Convert.ToDouble(ele[5]));
                    para_ma_term.Add(no, Convert.ToInt32(ele[8]));
                    para_nanpin_timing.Add(no, ele[9].Split(':').Select(double.Parse).ToArray());
                    para_nanpin_lot.Add(no, ele[10].Split(':').Select(double.Parse).ToArray());
                    no++;
                }
            }
            //do sim
            using (var sw = new StreamWriter("read sim.csv"))
            {
                sw.WriteLine("i,pt,lc,ma term,nanpin timing,nanpin lot,opt pl, opt num trade, opt win rate,earning performance,num trade performance,win rate performance");
                var progress = 0.0;
                ...
                for (int i = 0; i < no; i++)
                {
                    ...
                    var pl_performance = Math.Round(term_adjust * ac.performance_data.realized_pl - opt_pl[i],4);
                    var num_trade_performance = Math.Round(term_adjust * ac.performance_data.num_trade - opt_num_trade[i],4);
                    var win_rate_performance = Math.Round(ac.performance_data.win_rate / opt_win_rate[i], 4);
                    var res = i.ToString() + "," + para_pt[i].ToString() + "," + para_lc[i].ToString() + "," + para_ma_term[i].ToString() + "," + string.Join(":", para_nanpin_timing[i]) + "," +
                        string.Join(":", para_nanpin_lot[i]) + "," + pl_performance.ToString() + "," + opt_pl[i]+","+ opt_num_trade[i] +","+opt_win_rate[i] +","+num_trade_performance.ToString()+ "," + win_rate_performance.ToString();
